...
export class SymSpellClient {
	 ...
	 */
	createDictionaryEntryClientSide(key, count) {
	 ...
	 */
	async loadBigramDictionaryClientSide(data, termIndex, countIndex, separator = ' ') {
	 ...
	 */
	async loadDictionaryClientSide(data, termIndex, countIndex, separator = ' ') {
		const lines = data.split('\n'); // Split data into lines
	 ...
	 */
	async createDictionaryClientSide(dictFile) {
		try {
	...
	lookup(input, verbosity, maxEditDistance = null, { includeUnknown, ignoreToken, transferCasing } = {}) {
		...
		if (this.words.has(input)) {
			...
			if (verbosity !== SymSpellClient.Verbosity.ALL) {
				return earlyExit()
			}
		}
		...
		while (candidatePointer < candidates.length) {
				...
				for (let i = 0; i < dictSuggestions.length; i++) {
					...
					// number of edits in prefix ==maxediddistance  AND no identic suffix , then editdistance>maxEditDistance and no need for Levenshtein calculation (inputLen >= this.prefixLength) && (suggestionLen >= this.prefixLength)
						...
						if (
							...
							if (
								(
									verbosity !== SymSpellClient.Verbosity.ALL &&
					...
					if (distance <= maxEditDistance2) {
						...
						if (suggestions.length > 0) {
							switch (verbosity) {
								case SymSpellClient.Verbosity.CLOSEST: {
								...
								case SymSpellClient.Verbosity.TOP: {
			...
			if (lengthDiff < maxEditDistance && candidateLen <= this.prefixLength) {
				...
				if (verbosity !== SymSpellClient.Verbosity.ALL && lengthDiff >= maxEditDistance2) {
					continue
				}
	...
	lookupCompound(input, maxEditDistance = null, { ignoreNonWords, transferCasing } = {}) {
		...
		for (let i = 0; i < termList1.length; i++) {
			...
			suggestions = this.lookup(termList1[i], SymSpellClient.Verbosity.TOP, maxEditDistance)

			// combi check, always before split
			if (i > 0 && !lastCombi) {
				const suggestionsCombi = this.lookup(termList1[i - 1] + termList1[i], SymSpellClient.Verbosity.TOP, maxEditDistance)
				...
				if (suggestionsCombi.length > 0) {
					...
					if (
						...
						(
							...
							(
								suggestionsCombi[0].distance + 1 === distance1 &&
								suggestionsCombi[0].count > best1.count / SymSpellClient.N * best2.count
							)
						)
					) {
			...
			if (suggestions.length > 0 && (suggestions[0].distance === 0 || termList1[i].length === 1)) {
				...
				if (termList1[i].length > 1) {
					for (let j = 1; j < termList1[i].length; j++) {
						...
						const suggestions1 = this.lookup(part1, SymSpellClient.Verbosity.TOP, maxEditDistance)

						if (suggestions1.length > 0) {
							const suggestions2 = this.lookup(part2, SymSpellClient.Verbosity.TOP, maxEditDistance)
							...
							if (suggestions2.length > 0) {
								...
								if (this.bigrams.has(suggestionSplit.term)) {
									...
									suggestionSplit.count = Math.floor(Math.min(this.bigramCountMin, suggestions1[0].count / SymSpellClient.N * suggestions2[0].count))
								}
		...
		let count = SymSpellClient.N
		let s = ''

		suggestionParts.forEach((si) => {
			s += si.term + ' '
			count *= si.count / SymSpellClient.N
		})
	...
	// SymSpellClient.WordSegmentation uses a novel approach *without* recursion.
	...
	// SymSpellClient.WordSegmentation has a linear runtime O(n) to find the optimum composition
	...
	wordSegmentation(input, { maxEditDistance = null, maxSegmentationWordLength = null, ignoreToken } = {}) {
		...
		for (let j = 0; j < input.length; j++) {
			...
			for (let i = 1; i <= imax; i++) {
				...
				const results = this.lookup(part, SymSpellClient.Verbosity.TOP, maxEditDistance, { ignoreToken })
				...
				if (results.length > 0) {
					...
					// log(ab)=log(a)+log(b)
					topProbabilityLog = Math.log10(results[0].count / SymSpellClient.N)
				}
				else {
					topResult = part
					...
					topEd += part.length
					topProbabilityLog = Math.log10(10.0 / (SymSpellClient.N / Math.pow(10.0, part.length)))
				}
